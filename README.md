...
NormClaim ships with VS Code configuration that makes it immediately productive with [GitHub Copilot](https://github.com/features/copilot).

### Recommended extensions

When you open this repo in VS Code you will be prompted to install the workspace-recommended extensions. Accept the prompt, or install them manually:

| Extension | Purpose |
|-----------|---------|
| **GitHub Copilot** (`GitHub.copilot`) | AI inline code completion |
| **GitHub Copilot Chat** (`GitHub.copilot-chat`) | Ask Copilot questions about the codebase |
...
| Docker (`ms-azuretools.vscode-docker`) | Compose file editing & container management |
| YAML (`redhat.vscode-yaml`) | Syntax support for `docker-compose.yml` and FHIR YAML schemas |
| Thunder Client (`rangav.vscode-thunder-client`) | REST API testing inside VS Code |

### Using Copilot Chat

Copilot Chat is pre-loaded with project context from `.github/copilot-instructions.md`. You can ask it questions like:

- *"How does the Gemini extraction pipeline work?"*
- *"Write a new router that returns all missed diagnoses across all documents."*
- *"Explain the FHIR R4 Bundle structure generated by this project."*
- *"Fix the import error in `services/nlp_preprocessor.py`."*

Open the chat panel with **Ctrl + Shift + I** (Windows/Linux) or **⌘ + Shift + I** (macOS), or click the Copilot icon in the Activity Bar.
